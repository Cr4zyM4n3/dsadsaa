...
def get_group_funds(id, proxy_addr=None, timeout=5.0):
    ...
    sock.send(f"GET /v1/groups/{id}/currency HTTP/1.1\r\nHost: economy.roblox.com\r\n\r\n".encode())
...
def make_embed(group_info):
    return  dict(
      title="Claimable Group Found!",
        fields=[
             dict(name="General", value=f"**Name**: `{group_info['name']}`\n**MemberCount**: `{group_info['memberCount']}`\n**ClothingCount**: `{clothingcount(group_info['id'])}`   `{get_group_funds(group_info['id'])}` \n**Most Visits Game**: `{mostvisitegame(group_info['id'])}`\n\n"),
